...
# Set OpenAI API key
openai.api_key = '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'
...
for row_num, row in df.iterrows():
    combined_text += "\n" + " ".join([str(row[col]) for col in df.columns if col != sentiment_col and not pd.isna(row[col])])
    if len(combined_text) > MAX_TEXT_LENGTH:
        subtexts = [combined_text[i:i+MAX_TEXT_LENGTH] for i in range(0, len(combined_text), MAX_TEXT_LENGTH)]
        combined_text = subtexts[-1]
        for i in range(0, len(subtexts)-1):
            ...
            if api_requests_count > MAX_REQUESTS_PER_MINUTE:
                ...
                time.sleep(5)
            print("发送的文本1", subtexts[i].strip())
            sentiment = get_sentiment(subtexts[i].strip(), detect_type)
            df.loc[row_num-len(subtexts)+i+1:row_num, sentiment_col] = sentiment
    else:
        if row_num == len(df.index)-1:
            ...
            if api_requests_count > MAX_REQUESTS_PER_MINUTE:
                ...
                if loop_iteration % 10 == 0:
                    print("已完成", loop_iteration, "次迭代。")
                time.sleep(5)
            print("发送的文本2", subtexts[i].strip())
            sentiment = get_sentiment(combined_text.strip(), detect_type)
            df.loc[row_num-len(subtexts)+1:row_num+1, sentiment_col] = sentiment          
