...
def match_text(device: u2.Device, ocr_results: list, target_text: str, model_api_url: str, api_key: str, secret_key: str) -> bool:
    result_lines = [line for result in ocr_results for line in result]

    for box, (text, score) in result_lines:
        if text == target_text:
            x1, y1 = box[0][0], box[0][1]
            x2, y2 = box[2][0], box[2][1]
            print("ocr匹配到了关闭按钮文字", text)
            device.click((x2-x1)/2 + x1, (y2-y1)/2 + y1)
            return True
...
def main(config_path: str = "config.ini") -> None:
    ...
    def ocr_loop():
        while True:
            ...
            match_text(device, ocr_results, popup_text,
                       model_api_url, api_key, secret_key)
            time.sleep(5)
